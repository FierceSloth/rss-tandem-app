...
1. **`CodeEditor` (View)** — Редактор кода на базе CodeMirror. "Тупой" компонент, умеет только отдавать и принимать текст.
...
⚠️ **Архитектурное правило:** Согласно нашим гайдлайнам, мы строго разделяем View и Controller. **Никогда не пишите бизнес-логику (запросы к API, проверки тестов) внутри класса Страницы.** Страница должна быть "тупой".
